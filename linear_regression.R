...
lmer_coloc <- function(df, organism = FALSE) {
  lmer.coloc_grand_fit <- {
    if (organism) {
      lmer(
        Unique_Coloc ~ Probe * SampleType * Chemistry * Total_Read_Length + (1 | SampleID),
        data=df)
    }
    else {
      lmer(
        Unique_Coloc ~ Probe * Chemistry * Total_Read_Length + (1 | SampleID),
        data=df)
    }
  }
...
lmer_arg <- function(df, organism = FALSE) {
  lmer.arg_grand_fit <- {
    if (organism) {
      lmer(
        ARG ~ Probe * SampleType * Chemistry * Total_Read_Length + (1 | SampleID),
        data=df)
    }
    else {
      lmer(
        ARG ~ Probe * Chemistry * Total_Read_Length + (1 | SampleID),
        data=df)
    }
  }
...
lmer_mge <- function(df, organism = FALSE) {
  lmer.mge_grand_fit <- {
    if (organism) {
      lmer(
        MGE ~ Probe * SampleType * Chemistry * Total_Read_Length + (1 | SampleID),
        data=df)
    }
    else {
      lmer(
        MGE ~ Probe * Chemistry * Total_Read_Length + (1 | SampleID),
        data=df)
    }
  }
...
lmer_read <- function(df, organism = FALSE) {
  lmer.read_grand_fit <- {
    if (organism) {
      lmer(
        Read_Count ~ Probe * SampleType * Chemistry + (1 | SampleID),
        data=df)
    }
    else {
      lmer(
...
output_folder <- './output/regression_results/'
...
lmer_org_info_list <- c("mock_", "nonmock_")
df_list <- list(mock_df, non_mock_df)
